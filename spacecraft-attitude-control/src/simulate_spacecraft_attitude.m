function Project2(controller,varargin)
% Project2   run simulation of rotating satellite
%
%   Project2('FunctionName') uses the controller defined by
%       the function 'FunctionName' - for example, in a file with
%       the name FunctionName.m - in the simulation.
%
%   Project2('FunctionName','P1',V1,'P2','V2',...) optionally
...
%                         value or function used in the controller
%
%           'initcond': the desired initial condition to use in the
%                       simulation
% 
%           'vecplot': a vector to plot on the satellite's body frame other
%                      than the standard basis vectors
%
...
if ~isempty(find(strcmp(varargin,'reference'),1))
    ref = varargin{find(strcmp(varargin,'reference'))+1};
    if ~isa(ref,'function_handle')
        varargin{find(strcmp(varargin,'reference'))+1} = @(t) ref;
    end
end
addParameter(p,'reference',@(x) [1 0 0],@(x) isa(x,'function_handle'));
% addParameter(p,'reference',@(x)0,@(x) isa(x,'function_handle'));

addParameter(p,'initcond',[]);
addParameter(p,'vecplot',[]);
...
% Angular velocity
if ~isempty(process.initcond)
    process.w_01in1 = reshape(process.initcond,3,1);
else
    process.w_01in1 = 4*(randn(3,1)-.5);
end
...
try
    references = struct('r',process.reference(process.t));
catch exception
    warning(['The ''references'' function that was passed to\n' ...
             'Project04 threw the following error:\n\n' ...
             'threw the following error:\n\n' ...
             '==========================\n' ...
             '%s\n', ...
             '==========================\n\n' ...
...
if (isempty(fig))
    ...
    if isfield(process,'vecplot')
        v = process.vecplot/norm(process.vecplot);  
        pFrame = [0 1 0 0 v(1);
                  0 0 1 0 v(2);
                  0 0 0 1 v(3)];
    else
    ...
    % %%%%%%%%
    % fig.geom.pFrame1_in0 = [fig.geom.pFrame1_in0 reshape(process.reference(0),3,1)];
    % %%%%%%%%
...
%     fig.view0.frame0 = DrawFrame([],fig.geom.pFrame0_in0);
    ...
    axeslines = [fig.view0.frame1.x fig.view0.frame1.y fig.view0.frame1.z];
    legend(axeslines,'w_1 axis','w_2 axis','w_3 axis','fontsize',fs);
    ...
    if process.diagnostics
        ...
        set(fig.u.tau1,'xdata',t,'ydata',min(max(tau1,-process.tauMax),process.tauMax));
        set(fig.u.tau2,'xdata',t,'ydata',min(max(tau2,-process.tauMax),process.tauMax));
    end
...
if isempty(frame)
    ...
    if size(p,2)>4
        frame.v = plot3(p(1,[1 5]),p(2,[1 5]),p(3,[1 5]),'k-','linewidth',3);
    end
else
    ...
    if size(p,2)>4
        set(frame.v,'xdata',p(1,[1 5]),'ydata',p(2,[1 5]),'zdata',p(3,[1 5]));
    end
end
end
