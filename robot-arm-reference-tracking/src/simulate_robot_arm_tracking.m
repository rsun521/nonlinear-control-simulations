function Project3(controller,varargin)
% Project3   run simulation of underactuated robot arm
%
%   Project3('FunctionName') uses the controller defined by
%       the function 'FunctionName' - for example, in a file with
%       the name FunctionName.m - in the simulation.
%
%   Project3('FunctionName','P1',V1,'P2','V2',...) optionally
...
%                           will add an unknown disturbance to the torque
%
...
%                           @(t) sin(t)
%
%           'initcond' : a 4x1 matrix [q1 q2 v1 v2] that specifies the
...
addParameter(p,'tStop',35,@(x) isscalar(x) && isnumeric(x) && (x>0));
addParameter(p,'disturbance',false,@islogical);
addParameter(p,'reference',@(x)0,@(x) isa(x,'function_handle'));
addParameter(p,'initcond',[0.1; 0.01; 0.01; 0.01].*randn(4,1),...
...
% - Define reference for plotting
% process.reference = @(t) RefFunc(t);
...
process.t = 0;
% Joint angles
process.q1 = process.initcond(1,1);
process.q2 = process.initcond(2,1);
% Joint velocities
process.v1 = process.initcond(3,1);
process.v2 = process.initcond(4,1);
...
try
    ...
    warning(['The ''references'' function that was passed to\n' ...
             'Project03 threw the following error:\n\n' ...
             'threw the following error:\n\n' ...
             '==========================\n' ...
             '%s\n', ...
             '==========================\n\n' ...
...
if (isempty(fig))
    ...
    if process.diagnostics
        ...
        fig.x.legend = legend('q1','q2','v1','v2','r');
        xlabel('time');
