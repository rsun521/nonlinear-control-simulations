function MakeRoad(varargin)
% MakeRoad   create a road for use with DesignDemo06
%
%   MakeRoad creates a road of length 120 meters with piecewise-constant
%       curvature for use with DesignDemo06. It saves this road (by
%       default) to 'road.mat'.
%
%   MakeRoad('P1',V1,'P2','V2',...) optionally defines a number of
%       parameter values:
%
...
%
%           'filename : a string that, if defined, is the name of the file
%                       to contain the new road (default is 'road.mat')
%
...
addParameter(p,'filename','road.mat',@ischar);
% - Apply input parser
parse(p,varargin{:});
% - Extract parameters
params = p.Results;
...
road.roadwidth = params.roadwidth;
DrawRoad(road,params);
save(params.filename,'road');
fprintf(1,'road saved to ''%s''\n',params.filename);
